-- Bulk import support for taxpayer Excel workbooks.
-- The application validates the workbook before calling these service-role-only
-- functions. The database function keeps each commit batch atomic and makes
-- the taxpayer, source-year, queue, and activity records together.

create or replace function public.claim_refresh_jobs(p_tax_codes text[], p_limit integer default 10)
...
begin
  ...
  with normalized_codes as (
    select distinct regexp_replace(trim(input_code), '\s+', '', 'g') as tax_code
    from unnest(coalesce(p_tax_codes, array[]::text[])) as input(input_code)
    where input_code is not null
  ), candidate_rows as (
    select q.tax_code
    from public.refresh_queue q
    inner join normalized_codes c on c.tax_code = q.tax_code
...
revoke all on function public.claim_refresh_jobs(text[], integer) from public, anon, authenticated;
grant execute on function public.claim_refresh_jobs(text[], integer) to service_role;
